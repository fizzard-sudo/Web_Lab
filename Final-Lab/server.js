...
app.use(express.json()); // Support JSON-encoded bodies for API

app.use(session({
  secret: process.env.SESSION_SECRET || "my_secure_secret_key",
  resave: false,
...
app.get("/onsale-products", async (req, res) => {
  try {
    ...
    return res.status(500).send("Unable to load on-sale products right now.");
  }
});
...
app.get("/sale", (req, res) => res.redirect("/onsale-products"));
